...
import de.dj.djm.parser.impl.attributes.custom.Inverted;
import de.dj.djm.parser.impl.attributes.custom.Load;
import de.dj.djm.parser.impl.attributes.custom.Velocity;
...
import de.dj.djm.parser.impl.attributes.number.*;
...
public class AttributeGenerator {
    public static Attribute getAttributeFromString(String attribute) {
        if(!attribute.contains(EQUALS+"")) throw new RuntimeException("Could not split attribute: " + attribute);
        ...
        if(INVERTED.equals(name)) return new Inverted(Boolean.valueOf(value));
        ...
        if(VELOCITY.equals(name)) return new Velocity(Boolean.valueOf(value));
        ...
        if(SEND_RATIO.equals(name)) return new SendRatio(Integer.valueOf(value));
        ...
        if(VALUES.equals(name)) return new Values(value);
